...
import { addedToCart, selectCartQuantityById } from "../../features/cart/cartSlice";
...
export function ProductPage() {
    ...
    const quantity = useSelector(state => selectCartQuantityById(state, productId));
    ...
    return (
        <section className="product grid justify-between items-start gap-4 py-10">
            <div className="aspect-[3/4] rounded-md overflow-hidden">
                <img src={product.thumbnail} alt="product" className="img" />
            </div>
            <div>
                <h1 className="product-title">{product.title}</h1>
                <p className="mb-6 text-sm text-stone-400">{product.category}</p>
                <p className="mb-4 leading-snug tracking-tight">{product.description}</p>
            </div>
            <div className="flex flex-col gap-2 p-3 rounded-md border border-stone-600">
                <div className="flex items-center justify-between">
                    ...
                    <p className="tracking-tight text-stone-400">В наличии: {product.remains}</p>
                </div>
